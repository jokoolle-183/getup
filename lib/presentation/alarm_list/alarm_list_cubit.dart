...
class AlarmListCubit extends Cubit<AlarmListState> {
  ...
  AlarmListCubit(this._alarmSetRepository, this._regularAlarmRepository) : super(AlarmListState.initial()) {
    loadAlarms();
    checkAllPermissions();
  }
  ...
  Future<void> checkAndroidNotificationPermission() async {
    ...
    if (status.isDenied) {
      ...
      log('Notification permission ${res.isGranted ? '' : 'not '}granted');
    }
  }
  ...
  Future<void> _requestPermissions() async {
    ...
    if (status.isDenied) {}
    try {
      log("Requesting activity recognition permission...");
      final res = await Permission.activityRecognition.request();
      log("Activity recognition permission granted: ${res.isGranted}");
  ...
  Future<void> ignoreBatteryOptimizationsPermission() async {
    ...
    log('Ignore battery optimization permission: $status.');
    if (status.isDenied) {
      log('Requesting ignore battery optimization permission...');
      final res = await Permission.ignoreBatteryOptimizations.request();
      log('Ignore battery optimization permission ${res.isGranted ? '' : 'not'} granted');
    }
  }
  ...
  void openBatteryOptimizationSettings() {
    if (Platform.isAndroid) {
      ...
      intent.launch().catchError((e) {
        log("Error launching battery optimization settings: $e");
      });
    }
  }
}
